...
本技能用于把明确的可维护性、结构优化、现代化迁移或行为保持型性能优化目标，转成小步、可验证的代码修改。如果已有 `code-review` 发现，应优先复用：review finding 解释“为什么值得改”，本技能控制“怎样安全地改”。
...
如果重构触碰关键路径、公共 API、安全敏感代码或多个模块，建议后续再做一次 `code-review`。
